...
export default function InvestigationDetail() {
    ...
    // Forces the browser to reset scroll position strictly to top upon navigation
    useEffect(() => {
    ...
    useEffect(() => {
        const load = async () => {
            const rawData = await getInvestigationById(id);
            if (rawData) {
                setData({
                    ...rawData,
                    date: rawData.lastUpdate,
                    investigator: rawData.researcher
                });
            }
            setLoading(false);
        };
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-8 border-b border-slate-800">
                                ...
                                <div className="flex gap-4">
                                    {data.pdfUrl ? (
                                        <a href={data.pdfUrl} download target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-[#11ba82] text-black font-bold text-xs uppercase tracking-widest hover:bg-[#0fa675] transition-all rounded shadow-sm flex items-center gap-2">
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                            PDF
                                        </a>
                                    ) : (
                                        <button disabled className="px-6 py-3 bg-slate-800 text-slate-500 font-bold text-xs uppercase tracking-widest cursor-not-allowed rounded shadow-sm flex items-center gap-2" title="No se encuentra documento">
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                            Sin Documento
                                        </button>
                                    )}

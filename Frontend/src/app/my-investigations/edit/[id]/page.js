...
export default function EditarInvestigacionPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const investigationPayload = new FormData();
            investigationPayload.append('title', formData.title);
            investigationPayload.append('status', formData.status);
            investigationPayload.append('difficulty', formData.difficulty);
            ...
            investigationPayload.append('mentors', formData.mentors.split(',').map(m => m.trim()).filter(Boolean).join(','));

            if (formData.selectedFileRaw) {
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (file) {
                                                ...
                                                if (!allowedExtensions.includes(`.${extension}`)) {
                                                    alert("Formato no válido. Solo se permiten archivos PDF o Word (.doc, .docx)");
                                                    e.target.value = '';
                                                    return;
                                                }
                                                
                                                setFormData(prev => ({ ...prev, selectedFile: file.name, selectedFileRaw: file }));
                                            }
                                        }}
                                        accept=".pdf,.doc,.docx"
                                    />
